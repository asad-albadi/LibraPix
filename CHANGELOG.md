...
- Fixed large-library startup freezes on `Loading library snapshot` by chunking persisted snapshot apply on timed ticks (`SnapshotApplyTick`) and deferring reconcile/projection kickoff until chunked snapshot materialization is complete, avoiding a single large UI-thread hydrate apply burst.
